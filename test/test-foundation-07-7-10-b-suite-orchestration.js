...
const outcome=await processSuiteRunOrchestrationMessage({env:{SUITE_ORCHESTRATOR_CHILD_BATCH_SIZE:'2',SUITE_ORCHESTRATOR_CHILD_CONCURRENCY:'2',SUITE_ORCHESTRATOR_QUEUE:{send:async(m)=>continuations.push(m)}},message:{contractVersion:'qagent.suite-run-requested.v1',suiteRunId:'srun_x12345678',organizationId:'org_test',projectId,expectedCursor:0},deps:{
 ...
 getDispatch:async()=>({cursor:0,status:'PUBLISHED'}),markProcessing:async()=>{},getSuiteExecutionSlice:async()=>({suite:{inventoryFingerprint:'f'.repeat(64),endpointCount:3},items:[{ordinal:0,endpointId:'cep_a',testDesignVersionId:'tdv_a12345678',testDesignVersion:1,scenarioCount:2,scenarioIds:['test_001','test_002']},{ordinal:1,endpointId:'cep_b',testDesignVersionId:'tdv_b12345678',testDesignVersion:1,scenarioCount:1,scenarioIds:['test_001']}],nextOffset:2,hasMore:true}),
 upsertChild:async()=>{},createRun:async({input})=>{childRuns.push(input);return {run:{runId:`run_${childRuns.length}12345678`}};},markChildCreated:async()=>{},markChildError:async()=>{},advanceCursor:async(_env,args)=>{advanced=args;return true;},refresh:async()=>{}
}});
