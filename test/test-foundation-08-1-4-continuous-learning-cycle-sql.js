...
await upsertLearningScenario(env,{learningScenarioId:'lcs_sql',learningCycleId:'lcycle_sql',organizationId:'org_sql',projectId:'prj_sql',resultSetId:'rset_sql',runId:'run_sql',scenarioResultId:'sres_sql',scenarioId:'test_001',endpointId:'cep_sql',testDesignVersionId:'tdv_sql',testDesignVersion:7,outcome:'FAILED',httpOutcome:'RESPONSE',statusCode:422,assertionFailedCount:1,requestMethod:'POST',requestPath:'/v1/leave'});
...
const items=await listLearningScenarios(env,{organizationId:'org_sql',projectId:'prj_sql',learningCycleId:'lcycle_sql'});assert.equal(items[0].classification,'TEST_DATA_DRIFT');assert.equal(items[0].requestMethod,'POST');assert.equal(items[0].requestPath,'/v1/leave');assert.equal(items[0].verificationOutcome,'RECOVERED_BY_EVOLUTION');
