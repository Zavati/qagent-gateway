...
const inventory = {
  ...
  eligibilityPolicyVersion:'qagent.suite-execution-eligibility.v1', selectionPolicyVersion:'qagent.suite-selection-policy.v1.1',
  testDesignCount:2, endpointWithReadyCount:2, endpointWithExecutionEligibleCount:1,
  scenarioCount:5, readyScenarioCount:3, executionEligibleScenarioCount:2, policyBlockedReadyScenarioCount:1,
  blockedScenarioCount:2, reviewRequiredScenarioCount:1, needsDataScenarioCount:1, needsAuthScenarioCount:0,
  policyBlockedReasonCounts:{MUTATION_EXECUTION_DISABLED:1}, executable:true, readyAvailable:true,
  items:[], itemsTotal:2, itemsReturned:0, itemsTruncated:false, selectionIncluded:false, selection:[],
  projection:{mode:'IMMUTABLE_TEST_DESIGN_EXECUTION_PROJECTION',lazyBackfilledCount:0}, computedAt:'2026-08-24T20:00:00.000Z'
};
let forwarded = null;
...
assert.equal(got.executionEligibleScenarioCount,2);
assert.equal(got.policyBlockedReadyScenarioCount,1);
...
assert.match(forwarded.url,/\/test-inventory\?view=compact$/);

const suiteData = {
  contractVersion:'qagent.test-suite.v1', created:true, unchanged:false,
  suite:{suiteId:'suite_'+ 'b'.repeat(64),organizationId:'org_a',projectId:'prj_a',suiteType:'AUTO_PROJECT_READY',name:'Regressão automática',status:'ACTIVE',latestVersion:2,latestVersionId:'suitev_123',createdAt:'x',updatedAt:'x'},
  version:{contractVersion:'qagent.test-suite-version.v1',suiteVersionId:'suitev_123',suiteId:'suite_'+ 'b'.repeat(64),organizationId:'org_a',projectId:'prj_a',version:2,sourceType:'ZERO_CONFIG_PROJECT_READY',selectionPolicy:'LATEST_TEST_DESIGNS_EXECUTION_ELIGIBLE_SCENARIOS',selectionPolicyVersion:'qagent.suite-selection-policy.v1.1',inventoryFingerprint:'a'.repeat(64),testDesignCount:2,endpointCount:1,scenarioCount:2,selectionIncluded:false,selection:[],createdAt:'x'},
  inventory,
};
let materializeForwarded = null;
const materialized = await materializeAutoReadySuite({
  env:{}, organizationId:'org_a', projectId:'prj_a',
  fetchImpl: async (request) => { materializeForwarded=request; return new Response(JSON.stringify({status:'ok',data:suiteData}),{status:201}); },
});
assert.equal(materialized.version.scenarioCount,2);
assert.match(materializeForwarded.url,/\/materialize\?view=compact$/);
...
assert.equal(response.data.endpointWithExecutionEligibleCount,1);
...
console.log('Foundation 07.7.10-A FIX-1 Gateway Suite eligibility/compact bridge: PASS');
