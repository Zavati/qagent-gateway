import { baselineFromSource, validateObservedBaselineScenario, observedBaselineReady, canonicalBaselineJson } from '../baselineContract.js';
...
export async function materializeObservedBaselineRequests({ env, organizationId, projectId, endpointId, environmentId, scenarios, configuredBindings = [], loadSource = getCatalogObservedBaseline, now = Date.now() }) {
    ...
    for (const scenario of scenarios) {
        ...
        if (!observedBaselineReady(b, now))
        ...
        if (canonicalBaselineJson(current) !== canonicalBaselineJson(b))
