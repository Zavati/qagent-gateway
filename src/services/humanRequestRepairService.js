...
function normalizeInput(input){
  ...
  const changes=input.changes.map((raw,i)=>{
    ...
    const target=String(raw.target||'').trim().toUpperCase();if(target!=='BODY')fail('FIX-3 suporta somente BODY.','HUMAN_REQUEST_REPAIR_TARGET_UNSUPPORTED');
    const selector=text(raw.selector,'selector',320);if(!/^\$\.[A-Za-z_][A-Za-z0-9_-]*(?:\.[A-Za-z_][A-Za-z0-9_-]*){0,3}$/.test(selector)||SENSITIVE.test(selector))fail('Selector BODY não permitido para correção humana.','HUMAN_REQUEST_REPAIR_SELECTOR_FORBIDDEN');
    if(String(raw.operation||'').trim().toUpperCase()!=='SET_FIXED')fail('Operação de correção não suportada.','HUMAN_REQUEST_REPAIR_OPERATION_UNSUPPORTED');
    ...
    if(seen.has(selector))fail('Selector duplicado na correção.','HUMAN_REQUEST_REPAIR_CHANGE_DUPLICATE');seen.add(selector);
    return {target,selector,operation:'SET_FIXED',valueType,value:raw.value};
  });
...
export async function createHumanRequestRepairV1({env,organizationId,projectId,userId,input,deps={}}={}){
  ...
  const diagnosticFields=new Map((inspected.requestIssue.fields||[]).filter((f)=>f?.target==='BODY'&&f?.selector).map((f)=>[f.selector,f]));
  ...
  const bindings=scenario?.spec?.testData?.bindings||[];const bodyEvidence=new Map((resultScenario?.evidence?.request?.bodyFields||[]).map((f)=>[f.path,f]));
  const registryChanges=[];
  for(const change of normalized.changes){
    const diagnostic=diagnosticFields.get(change.selector);if(!diagnostic)fail('Somente campos identificados pelo diagnóstico podem ser corrigidos nesta tela.','HUMAN_REQUEST_REPAIR_FIELD_NOT_DIAGNOSED',409,{selector:change.selector});
  ...
  try{
    for(const change of normalized.changes){
      const current=exactEndpointBinding(existing,rs.environmentId,'BODY',change.selector);
