...
export async function createHumanRequestRepairV1({env,organizationId,projectId,userId,input,deps={}}={}){
  ...
  try{
    ...
    await (deps.recordLearning||recordHumanRepairInLearningCycle)(env,{organizationId,projectId,resultSetId:normalized.resultSetId,scenarioId:normalized.scenarioId,repairId,testDesignVersionId:derived.testDesign.versionId,testDesignVersion:derived.testDesign.version,rerunRunId:rerun?.run?.runId||null,rerunRequested:rerun?.requested===true,rerunStatus:rerun?.status||null,rerunErrorCode:rerun?.errorCode||null}).catch(()=>{});
