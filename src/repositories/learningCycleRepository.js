...
export async function updateLearningScenarioInspection(env,{learningCycleId,resultSetId,scenarioId,eligible,reason,requestIssueDetected,effectiveState}){const db=requireDataDb(env),now=new Date().toISOString();await db.prepare(`UPDATE continuous_learning_scenarios SET inspection_state='ANALYZED',inspection_eligible=?,inspection_reason=?,request_issue_detected=?,effective_state=?,analyzed_at=COALESCE(analyzed_at,?),updated_at=? WHERE learning_cycle_id=? AND source_result_set_id=? AND scenario_id=?`).bind(eligible?1:0,reason||null,requestIssueDetected?1:0,effectiveState,now,now,learningCycleId,resultSetId,scenarioId).run();}

export async function updateLearningScenarioProposal(env,{learningCycleId,resultSetId,scenarioId,proposalId,classification,decision,confidence,riskScore,riskLevel,autoAction,evolvedTestDesignVersionId,evolvedTestDesignVersion,evolutionRerunRunId,effectiveState}){const db=requireDataDb(env),now=new Date().toISOString();await db.prepare(`UPDATE continuous_learning_scenarios SET proposal_id=?,classification=?,decision=?,confidence=?,risk_score=?,risk_level=?,auto_action=?,evolved_test_design_version_id=?,evolved_test_design_version=?,evolution_rerun_run_id=?,effective_state=?,analyzed_at=COALESCE(analyzed_at,?),updated_at=? WHERE learning_cycle_id=? AND source_result_set_id=? AND scenario_id=?`).bind(proposalId||null,classification||null,decision||null,confidence??null,riskScore??null,riskLevel||null,autoAction||null,evolvedTestDesignVersionId||null,evolvedTestDesignVersion??null,evolutionRerunRunId||null,effectiveState,now,now,learningCycleId,resultSetId,scenarioId).run();}

export async function updateLearningScenarioVerificationByProposal(env,{proposalId,outcome,reasonCode,rerunRunId,effectiveState}){const db=requireDataDb(env),now=new Date().toISOString();const row=mapScenario(await db.prepare(`${SCENARIO_SELECT} WHERE proposal_id=? LIMIT 1`).bind(proposalId).first());if(!row)return null;await db.prepare(`UPDATE continuous_learning_scenarios SET evolution_rerun_run_id=COALESCE(?,evolution_rerun_run_id),verification_outcome=?,verification_reason_code=?,effective_state=?,verified_at=?,updated_at=? WHERE learning_scenario_id=?`).bind(rerunRunId||null,outcome||null,reasonCode||null,effectiveState,now,now,row.learningScenarioId).run();return mapScenario(await db.prepare(`${SCENARIO_SELECT} WHERE learning_scenario_id=?`).bind(row.learningScenarioId).first());}

export async function updateLearningScenarioHumanRepair(env,{learningCycleId,resultSetId,scenarioId,repairId,testDesignVersionId,testDesignVersion,humanRerunRunId,effectiveState}){const db=requireDataDb(env),now=new Date().toISOString();await db.prepare(`UPDATE continuous_learning_scenarios SET human_repair_id=?,human_repair_test_design_version_id=?,human_repair_test_design_version=?,human_rerun_run_id=?,effective_state=?,updated_at=? WHERE learning_cycle_id=? AND source_result_set_id=? AND scenario_id=?`).bind(repairId,testDesignVersionId||null,testDesignVersion??null,humanRerunRunId||null,effectiveState,now,learningCycleId,resultSetId,scenarioId).run();return mapScenario(await db.prepare(`${SCENARIO_SELECT} WHERE learning_cycle_id=? AND source_result_set_id=? AND scenario_id=? LIMIT 1`).bind(learningCycleId,resultSetId,scenarioId).first());}

export async function updateLearningScenarioHumanOutcomeByRerun(env,{humanRerunRunId,effectiveState,verificationOutcome,reasonCode}){const db=requireDataDb(env),now=new Date().toISOString();const row=mapScenario(await db.prepare(`${SCENARIO_SELECT} WHERE human_rerun_run_id=? LIMIT 1`).bind(humanRerunRunId).first());if(!row)return null;await db.prepare(`UPDATE continuous_learning_scenarios SET verification_outcome=?,verification_reason_code=?,effective_state=?,verified_at=?,updated_at=? WHERE learning_scenario_id=?`).bind(verificationOutcome,reasonCode, effectiveState,now,now,row.learningScenarioId).run();return mapScenario(await db.prepare(`${SCENARIO_SELECT} WHERE learning_scenario_id=?`).bind(row.learningScenarioId).first());}
