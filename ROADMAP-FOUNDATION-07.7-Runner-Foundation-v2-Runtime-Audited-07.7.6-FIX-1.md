...
```text
EXPLICIT_CONFIG ou um DISCOVERED_OBSERVATION único/seguro pode produzir READY; ambiguidade continua bloqueando
```
...
## 07.7.6 — HTTP Executor v1 ✅ implemented / production transport activated; successful response gate pending
...
Production observation on Buggy Cars proved the executor reached `fetch()` (`httpRequestCount=1`) but received no HTTP response (`httpResponseCount=0`, `httpNetworkErrorCount=1`). This triggered FIX-1 before Assertion Engine.

---

## 07.7.6 FIX-1 — HTTP Network Diagnostics ✅ implemented / production diagnostic gate pending

Add safe diagnostics without turning Gateway D1 into Results Plane:

```text
per-scenario structured Runner logs
network categories: DNS / CONNECT / TLS / RESET / ABORT / FETCH / UNKNOWN
response status-class counts: 2xx / 3xx / 4xx / 5xx
one bounded primary diagnostic per attempt
raw Error.message forbidden from persistence/log envelope
```

Important semantic rule:

```text
HTTP 500 = RESPONSE + 5xx diagnostic
network failure before a response = NETWORK_ERROR
```

Gate:

```text
repeat Buggy Cars GET /prod/models
-> if network failure: identify safe category/code/name/cause
-> if upstream responds: expose exact primary non-success status + class counts
```
...
# 07.7.6-A — Zero-Config Runtime Bootstrap v1
...
else unique safe observed HTTPS origin -> DISCOVERED_OBSERVATION
else NEEDS_ENVIRONMENT
```

For discovered runtime:

```text
Test Design may be READY without a persisted API Service.
Create Run requires explicit confirmation of the discovered target.
The confirmed Run freezes the discovered Base URL in qagent.runtime-snapshot.v1.
No API Service record is silently created.
```

First production gate: Buggy Cars `GET /prod/models` with zero configured API Services.
